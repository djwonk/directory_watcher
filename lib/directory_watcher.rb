...
#    dw.start
#    gets # wait for user to press enter before terminating
#    dw.stop
#
...
#    dw = DirectoryWatcher.new('.')
#    dw.glob = '**/*.rb'
#    dw.reset(true) # +true+ flag forces a pre-load of the files
#
...
class DirectoryWatcher
  ...
  #    :modified =>  file has been modified (mtime and/or size have changed)
  ...
  def self.libpath(*args, &block)
    rv = if args.empty?
      ...
      ::File.join(LIBPATH, args.flatten)
    end
    if block
      begin
  ...
  def glob=(val)
    ...
    when String
      [File.join(@dir, val)]
    when Array
      val.flatten.map! { |g| File.join(@dir, g) }
    else
      raise(ArgumentError, 'expecting a glob pattern or an array of glob patterns')
    end
    glob.uniq!
    @scanner.glob = glob
  end
  ...
  def persist!
    return if running?
    if @persist
      File.open(@persist, 'w') do |fd|
        fd.write(YAML.dump(@scanner.files))
      end
    end
    self
  end
  ...
  # Is the directory watcher currently running?
  def running?
    @scanner.running?
  end

  # Start the directory watcher scanning thread (unless already running).
  def start
  ...
  # Stop the directory watcher scanning thread (unless already stopped).
  def stop
  ...
  # the file list to be pre-loaded after it has been cleared, thus skipping
